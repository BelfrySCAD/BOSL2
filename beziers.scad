...
//   a position with no curvature, because 1/0 is not a number.
// Arguments:
//   bezier = The list of control points that define the Bezier curve.
//   u = Parameter values for evaluating the curve, given as a single value, a list or a range.
...
//   as single numbers you get back a single point. If you give u and v as lists or ranges you
//   get a 2d rectangular array of points. If one but not both of u and v is a list or range then you
...
//   You must also ensure that the patches mate exactly along their edges, otherwise rendering errors result from the invalid VNF.  
// Arguments:
...
//   occurs when the tangents are parallel, or nearly parallel.  In this case a unit vector is returned, but not
